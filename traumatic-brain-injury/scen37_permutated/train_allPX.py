...
'''
Commenting out to permutated labels
...
'''
# Permutating labels to test for random classification performance. 
PX_labels = ['TBI01PX03','TBI06PX03','TBI09PX03','TBI13PX03','TBI14PX03','TBI16PX03','TBI20PX03','TBI22PX03','TBI23PX03','TBI07PX01', 'TBI08PX01', 'TBI10PX01', 'TBI11PX01', 'TBI12PX01', 'TBI17PX01', 'TBI19PX01', 'TBI24PX01', 'TBI25PX01', 'TBI27PX01']
shuffled_labels = random.sample(PX_labels, len(PX_labels))
Control_cases = shuffled_labels[:10] # First 10 PX labels from shuffled list are controls. 
...
#
cases = ds.removeOverlappingKmers(cases, controls) # removed holdout 
...
#
samples_train, samples_val = ds.split_samples_TrainAllPX(samples, args.holdouts) 
# all samples go into samples_train. samples_val still has one sample, but its presence doesn't affect the training since the best fit is selected based on the minimum cross entropy loss of the training data. 
...
# LR Scheduler
#
scheduler = torch.optim.lr_scheduler.OneCycleLR(optimizer, max_lr = learning_rate, epochs = 2049, steps_per_epoch = len(samples_train))
