...
# Updated for TBI IGH project by: Annie Needs
# Date Updated: 2025-12-01
# Changes: edited load_cdr3s funtion to accomodate MiXCR ouput format for IGH sequences
##########################################################################################
...
def load_cdr3s(path_tsv, min_length=4, max_length=32, seq_type='IGH'):
  receptors = {}
  with open(path_tsv, 'r') as stream:
    ...
    for row in reader:
      if seq_type == 'IGH':
        cdr3 = row['aaSeqCDR3']
#        quantity = float(row['readFraction'])
        quantity = float(row['readCount'])
      elif seq_type == 'v3':
        print('IGK & IGL or TCR not yet supported for TBI project')
        cdr3 = row['amino_acid']
#        quantity = float(row['frequency'])
        ...
        exit()
      if min_length <= len(cdr3) and len(cdr3) <= max_length and quantity > 0.0 and 'X' not in cdr3 and '*' not in cdr3 and '_' not in cdr3:
